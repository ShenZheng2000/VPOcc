...
class GaussianVPGrid(nn.Module, RecasensSaliencyToGridMixin):
    """
    Grid generator that builds a Gaussian saliency map centered at the vanishing point.
    sigma controls how large the emphasized VP region is.
    Larger sigma -> larger emphasized region / weaker concentration.
    Smaller sigma -> tighter focus around VP.
    """

    def __init__(self, input_shape, output_shape, sigma=120.0, **kwargs):
        super(GaussianVPGrid, self).__init__()
        RecasensSaliencyToGridMixin.__init__(self, output_shape, **kwargs)

        self.im_shape = tuple(input_shape)      # (H, W)
        self.output_shape = tuple(output_shape)
        self.sigma = sigma

    def forward(self, imgs, v_pts, **kwargs):
        """
        imgs:  [B, C, H, W]
        v_pts: [B, 2] or [2], in (x, y) pixel coordinates
        """
        device = imgs.device
        B, _, H, W = imgs.shape

        if v_pts.dim() == 1:
            v_pts = v_pts.unsqueeze(0).repeat(B, 1)
        elif v_pts.dim() == 3:
            # just in case input comes as [B, 1, 2]
            v_pts = v_pts.squeeze(1)

        v_pts = v_pts.to(device).float()
        ...
        vp_x = v_pts[:, 0].view(B, 1, 1)
        vp_y = v_pts[:, 1].view(B, 1, 1)

        # Gaussian saliency centered at VP
        dist2 = (xs - vp_x) ** 2 + (ys - vp_y) ** 2
        saliency = torch.exp(-dist2 / (2 * (self.sigma ** 2)))   # [B, H, W]

        # normalize each sample to sum to 1
        saliency = saliency / (saliency.sum(dim=(1, 2), keepdim=True) + 1e-6)

        # [B, 1, H, W]
        self.saliency = saliency.unsqueeze(1)

        # keep same convention as CuboidGlobalKDEGrid
        self.saliency = F.interpolate(
            self.saliency, (31, 51), mode='bilinear', align_corners=True
        )

        if self.separable:
            x_saliency = self.saliency.sum(dim=2)   # [B, 1, W]
            y_saliency = self.saliency.sum(dim=3)   # [B, 1, H]
            grid = self.separable_saliency_to_grid(
                imgs, x_saliency, y_saliency, device
            )
        else:
            grid = self.nonseparable_saliency_to_grid(
                imgs, self.saliency, device
            )
